...
def sync_document_links(en_names):
    ...
    Invoice = HANYA invoice yang benar-benar menarik Expense Note ini lewat tabel
    Reimburse Items. Invoice yang cuma menagih JOB-nya (Shipping/Packing List sama)
    SENGAJA tidak ikut: satu invoice job bikin semua Expense Note di job itu seolah
    sudah ditagih, padahal biayanya tidak pernah ditarik ke mana pun.
    """
    for en in {n for n in (en_names or []) if n}:
        ...
        frappe.db.set_value(
            ...
            en,
            {
                "invoice_no": ", ".join(sorted(set(_reimburse_invoices(en)))) or None,
