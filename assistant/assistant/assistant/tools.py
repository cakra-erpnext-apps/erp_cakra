...
def check_duplicate(packing_list_no=None, external_ref=None, shipping_list_no=None):
	...
	# Shipping List: nomor = name dokumen (tidak ada field shipping_list_no lagi).
	if shipping_list_no and frappe.db.exists(SL_DOCTYPE, shipping_list_no):
		hits.append({"field": "name", "value": shipping_list_no, "name": shipping_list_no})
	if external_ref:
...
def create_shipping_list_draft(fields):
	...
	# Nomor Shipping List = name dokumen, dibuat otomatis oleh autoname saat Save/Confirm.
	auto_no = True
