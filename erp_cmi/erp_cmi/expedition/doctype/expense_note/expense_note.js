...
function load_pl_containers(frm) {
	...
	frappe.call({
		method: 'erp_cmi.expedition.doctype.expense_note.expense_note.get_packing_containers',
...
function load_sl_bls(frm) {
	...
	frappe.call({
		method: 'erp_cmi.expedition.doctype.expense_note.expense_note.get_shipping_list_bls',
...
function load_bl_containers(frm) {
	...
	frappe.call({
		method: 'erp_cmi.expedition.doctype.expense_note.expense_note.get_bl_containers',
...
// /assets/erp_cmi tidak tersaji (symlink rusak di nginx frontend, tanpa bench build). ----
...
function cmi_charges_init(frm) {
	...
	frappe.db.get_list('Expense Class', {
		filters: { disabled: 0 }, fields: ['name', 'account_1'], limit: 0, order_by: 'class_name asc',
	}).then((rows) => {
...
function cmi_charges_class_modal(frm, editIndex) {
	...
	const used = (frm._charges || []).map((p) => p.cls).filter((c) => !existing || c !== existing.cls);
	const sizeOf = {};
		...
		fields: [
			{
				...
				get_query: () => ({ filters: { disabled: 0, name: ['not in', used] } }),
			},
...
// Subtotal berubah (charges sync) → re-parse semua input. Input "%" otomatis
// menyesuaikan nominal; input nominal tetap, persennya yang disesuaikan.
function cmi_recalc_amounts(frm) {
	['tax', 'pph', 'pph22', 'discount'].forEach((k) => cmi_parse_adj(frm, k));
	cmi_recalc_net(frm);
}
// Record lama (punya *_amount/*_pct tapi *_input kosong): isi *_input dari data lama.
function cmi_backfill_inputs(frm) {
	const b = cmi_base(frm);
	['tax', 'pph', 'pph22', 'discount'].forEach((k) => {
		if (String(frm.doc[k + '_input'] || '').trim()) return;
		const amt = flt(frm.doc[k + '_amount']), pct = flt(frm.doc[k + '_pct']);
		if (pct && b && Math.abs(b * pct / 100 - amt) < 0.5) frm.doc[k + '_input'] = cmi_fmt(pct) + '%';
		else if (amt) frm.doc[k + '_input'] = String(amt);
	});
}
...
function cmi_charges_sync(frm) {
	...
	(frm._charges || []).forEach((p) => {
		Object.keys(p.cont).forEach((cno) => {
			...
			if (cls.account_1) row.expense_account = cls.account_1;
