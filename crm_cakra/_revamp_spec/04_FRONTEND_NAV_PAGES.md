...
| `/deals/view/:viewType?` | `/deals` | `Deals` | `Deals.vue` | — | **meta.label = 'Inquiry'** **[CUSTOM relabel]** |
| `/deals/new` | — | `NewDeal` | `DealNew.vue` | — | Full-page create form |
| `/deals/:dealId` | — | `Deal` | `Deal.vue` **or** `MobileDeal.vue` | `true` | Mobile if `<768` |
...
**Mobile-vs-desktop routing:** done at route-config time via `handleMobileView(name)` returning `Mobile${name}` when `window.innerWidth < 768`. Applies to `Lead`, `Deal`, `Contact`, `Organization` detail pages only. (Note: layout selection in App.vue uses a `<640` breakpoint; detail-component selection uses `<768`.)
...
7. **Detail tab restore:** for `Deal`/`Lead` with no hash → append `#<lastTab>` from `localStorage['lastDealTab'|'lastLeadTab']` (default `#activity`).
...
`Leads→CRM Lead`, `Deals→CRM Deal`, `Contacts→Contact`, `Organizations→CRM Organization`, `Quotations→CRM Quotation`, `Estimations→CRM Estimation`, `Notes→FCRM Note`, `Tasks→CRM Task`, `Call Logs→CRM Call Log`.
...
> **[CUSTOM relabels]** vs stock Frappe CRM: "Organizations" → **Accounts**, "Deals" → **Inquiries**. "Quotations" and "Estimations" are custom-added entries. (Stock sidebar order is Leads/Deals/Contacts/Organizations/Notes/Tasks/Call Logs.)
...
**Onboarding steps** (driven by `useOnboarding('frappecrm')`, surfaced in GettingStartedBanner/HelpModal): setup password, create first lead, invite team (managers), convert lead→deal, create first task, create first note, add first comment, send first email, change deal status. Each step deep-links into the relevant page/tab and fires telemetry.
...
> **Note (inconsistency):** the mobile sidebar's hard-coded link list is the **stock** set/labels: Leads, Deals, Contacts, Organizations, Notes, Tasks, Call Logs — it does **not** include Dashboard/Quotations/Estimations and does **not** apply the "Accounts"/"Inquiries" relabels. Desktop and mobile nav differ.
...
- **Inquiries (Deals)** → routes to `NewDeal` (full-page form). Kanban "+" in a column passes the column's status as a query default.
...
All list pages share the same skeleton (Leads/Deals/Quotations/Estimations/Contacts/Organizations/Tasks/CallLogs use it; Notes is a card variant):
...
- **Convert to Deal** (leads only) → `crm.fcrm.doctype.crm_lead.crm_lead.convert_to_deal` per selected lead.
...
- **Actions:** edit/save fields (`document.save`/`triggerOnChange`), change **status** (dropdown; Lost flow via `LostReasonModal`), **Convert to Deal** (`ConvertToDealModal`, hidden once `doc.converted`), **Assign** (`AssignTo`), **Void/Unvoid** `crm.api.void.void_document` **[CUSTOM]**, make call, send email (`openEmailBox`), open website, attach file (`FilesUploader`), delete (`DeleteLinkedDocModal`).
- **[CUSTOM]** When `doc.converted` flips true, a watcher forces all fields `read_only:1`.

### Deals / "Inquiries" (`Deals.vue`) — route `Deals` (label **Inquiry**), doctype **CRM Deal**
- Identical list pattern to Leads (status from `CRM Deal Status`, org logos from `organizationsStore`).
- **Create** routes to `NewDeal` (not a modal). Per-row dropdown: Make a Call, New Note, New Task (reference = CRM Deal). Kanban "+" → `NewDeal` with column-field query.

### Deal (`Deal.vue` / `MobileDeal.vue`) — route `Deal`, doctype **CRM Deal**
- **Loads:** `useDocument('CRM Deal', dealId)`; lazy `useDocument('CRM Organization', org)` for logo; side panel `get_sidepanel_sections`; **contacts** via `crm.fcrm.doctype.crm_deal.api.get_deal_contacts`.
- **Tabs:** same set as Lead; active tab in `localStorage['lastDealTab']`.
- **Side panel extras:** organization avatar; a **Contacts section** listing linked contacts with primary badge + per-contact dropdown.
- **Actions:** edit/save fields; change status (Lost flow; onboarding `change_deal_status`); Void/Unvoid **[CUSTOM]**; **add/remove/set-primary contact** (`crm_deal.add_contact` / `remove_contact` / `set_primary_contact`); create Organization (`OrganizationModal`) / Contact (`ContactModal`); call primary contact; send email; attach; delete; assign. Socket `crm_customer_created`; broadcast `reload-deal-sections`.

### NewDeal (`DealNew.vue`) — route `NewDeal`, doctype **CRM Deal**
- **Loads:** `useDocument('CRM Deal')` (new); field layout `...get_fields_layout` (type "Data Fields"). Renders via `FieldLayout`.
- **Create:** `frappe.client.insert` → push to `Deal`. Cancel → `Deals`.
- **[CUSTOM] onMount defaults:** seeds from `route.query` (kanban), `deal_owner`=current user, `status`=first deal status, `currency='IDR'`, `exchange_rate=1`.
...
- **Loads:** `useDocument('CRM Quotation')`; `...get_fields_layout`; **available inquiries** via `crm.api.quotation.get_available_inquiries` (Won deals not yet quoted) applied as a link filter on `inquiry`.
- Selecting an inquiry auto-fills account/subject via `frappe.client.get_value` on `CRM Deal`.
...
- **Loads:** `useDocument('Contact', id)`; linked deals `crm.api.contact.get_linked_deals`; side panel `get_sidepanel_sections`.
- **Layout:** left side panel (image, multi-email/multi-phone dropdowns, address); right `Tabs` = **Deals** only (`tabIndex` ref, not persisted).
...
- **Loads:** `useDocument('CRM Organization', id)`; `get_sidepanel_sections`; linked `createListResource('CRM Deal', {organization})` and `createListResource('Contact', {company_name})`.
- **Tabs:** Deals, Contacts (`tabIndex`, not persisted).
...
- **Actions:** Create (defaults status `Backlog`, priority `Low`) / edit / **Delete** `frappe.client.delete`; reference-doc button routes to `Deal`/`Lead`. Deep-link `?open=<task>` (int). Onboarding `create_first_task`.
...
- Thin wrapper over frappe-ui's `DataImport` component (upload → map → run import). `doctypeMap` covers CRM Lead, CRM Deal, Contact, CRM Task, CRM Organization, CRM Call Log (each with list/page routes).
...
- Notifications list from `notificationsStore`. Click → `mark_doc_as_read` + navigate (Lead/Deal anchor via hash). "Mark all as read". Socket `crm_notification` triggers reload.
...
| `statuses.js` (`crm-statuses`) | Pinia | listResources for `CRM Lead Status`, `CRM Deal Status`, `CRM Communication Status` (name/color/position/type). `getLeadStatus/getDealStatus`, `statusOptions(doctype, statuses, onChange)` (builds dropdown w/ indicator color + telemetry). |
...
| Users / orgs / statuses | `crm.api.session.get_users`, `crm.api.session.get_organizations`, list of `CRM Lead/Deal/Communication Status` |
| Lead→Deal convert | `crm.fcrm.doctype.crm_lead.crm_lead.convert_to_deal` |
| Deal contacts | `crm.fcrm.doctype.crm_deal.api.get_deal_contacts`; `crm_deal.{add_contact,remove_contact,set_primary_contact}` |
| Contact multi-value | `crm.api.contact.{get_linked_deals,set_as_primary,create_new}` |
| Quotation **[CUSTOM]** | `crm.api.quotation.get_available_inquiries`; `...crm_quotation.convert_to_estimation` |
| Void **[CUSTOM]** | `crm.api.void.void_document` (Lead, Deal, Quotation) |
...
| Onboarding | `crm.api.onboarding.{get_first_lead,get_first_deal}` |
...
- **Mobile vs desktop** is decided client-side at load by viewport width (640 for layout, 768 for Lead/Deal/Contact/Organization detail components). A Go-templated UI can decide server-side or keep two templates.
