<template>
  <ListView :class="$attrs.class" :columns="columns" :rows="rows" :options="{
    ...
    <ListRows v-slot="{ idx, column, item, row }" :rows="rows" :doctype="doctype">
...
defineProps({
  ...
  // Master sederhana lain (mis. CRM Cost Component) memakai list view yang sama.
  doctype: { type: String, default: 'CRM Product' },
  options: {
    type: Object,
    default: () => ({
      selectable: true,
