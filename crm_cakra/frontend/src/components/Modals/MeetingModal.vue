    ...
    <template #body-content>
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <LinkField label="Host" doctype="User" v-model="form.host" />
        ...
        <div class="grid grid-cols-2 gap-4">
          <FormControl v-model="form.meeting_from" label="From" type="datetime-local" />
          <FormControl v-model="form.meeting_to" label="To" type="datetime-local" />
        </div>
        <FormControl v-model="form.location" label="Location" />
        <FormControl v-model="form.purpose" label="Tujuan Visit" type="textarea" />
        <div class="grid grid-cols-2 gap-4">
          <LinkField label="Marketing" doctype="User" v-model="form.marketing" />
...
const blank = () => ({
  subject: '', host: user, status: 'Scheduled', organization: '', contact: '',
  inquiry: '', quotation: '', meeting_from: '', meeting_to: '', location: '',
  purpose: '', marketing: '', venue: '', provider: '', nominal: 0,
})
...
const insertDoc = createResource({
  ...
  makeParams: () => ({ doc: { doctype: 'CRM Meeting', ...form.value } }),
  onSuccess: onSaved,
  onError,
})
const updateDoc = createResource({
  url: 'frappe.client.set_value',
  makeParams: () => ({ doctype: 'CRM Meeting', name: props.meetingId, fieldname: form.value }),
  onSuccess: onSaved,
  onError,
})
