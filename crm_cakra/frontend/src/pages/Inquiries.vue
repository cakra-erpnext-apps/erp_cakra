<template>
  <LayoutHeader>
    <template #left-header>
      <ViewBreadcrumbs v-model="viewControls" routeName="Inquiries" />
    </template>
    <template #right-header>
      <CustomActions
        v-if="inquiriesListView?.customListActions"
        :actions="inquiriesListView.customListActions"
      />
      <Button
        variant="solid"
        :label="__('Create')"
        iconLeft="plus"
        @click="router.push({ name: 'NewInquiry' })"
      />
    </template>
  </LayoutHeader>
  <ViewControls
  ...
  >
    ...
    <template #fields="{ fieldName, itemName }">
      ...
      >
        ...
        <div v-else-if="fieldName === 'inquiry_owner'">
          <Avatar
  ...
  <InquiriesListView
    v-else-if="inquiries.data && rows.length"
    ref="inquiriesListView"
    v-model="inquiries.data.page_length_count"
    v-model:list="inquiries"
    :rows="rows"
    :columns="columns"
    :options="{
      showTooltip: false,
      resizeColumn: true,
      rowCount: inquiries.data.row_count,
      totalCount: inquiries.data.total_count,
    }"
    @loadMore="() => loadMore++"
...
const inquiriesListView = ref(null)
const showInquiryModal = ref(false)

const defaults = reactive({})

// inquiries data is loaded in the ViewControls component
const inquiries = ref({})
...
// Rows
const rows = computed(() => {
  if (!inquiries.value?.data?.data) return []
  if (inquiries.value.data.view_type === 'group_by') {
    if (!inquiries.value?.data.group_by_field?.fieldname) return []
    return getGroupedByRows(
      inquiries.value?.data.data,
      inquiries.value?.data.group_by_field,
      inquiries.value.data.columns,
    )
  } else if (inquiries.value.data.view_type === 'kanban') {
    return getKanbanRows(inquiries.value.data.data, inquiries.value.data.fields)
  } else {
    return parseRows(inquiries.value?.data.data, inquiries.value.data.columns)
  }
})
...
function getGroupedByRows(listRows, groupByField, columns) {
  ...
  groupByField.options?.forEach((option) => {
    ...
    if (groupByField.fieldname == 'status') {
      groupDetail.icon = () =>
        h(IndicatorIcon, {
          class: getInquiryStatus(option)?.color,
        })
    }
...
function parseRows(rows, columns = []) {
  let view_type = inquiries.value.data.view_type
  let key = view_type === 'kanban' ? 'fieldname' : 'key'
  let type = view_type === 'kanban' ? 'fieldtype' : 'type'

  return rows.map((inquiry) => {
    let _rows = {}
    inquiries.value.data.rows.forEach((row) => {
      _rows[row] = inquiry[row]
      ...
      if (
        ...
        _rows[row] = formatDate(inquiry[row], '', true, fieldType == 'Datetime')
      }

      if (fieldType && fieldType == 'Currency') {
        _rows[row] = getFormattedCurrency(row, inquiry)
      }

      if (fieldType && fieldType == 'Float') {
        _rows[row] = getFormattedFloat(row, inquiry)
      }

      if (fieldType && fieldType == 'Percent') {
        _rows[row] = getFormattedPercent(row, inquiry)
      }
      ...
      if (row == 'organization') {
        ...
        _rows[row] = website(inquiry.website)
      } else if (row == 'status') {
        _rows[row] = {
        ...
        let value = inquiry.sla_status
        let tooltipText = value
        let color =
          inquiry.sla_status == 'Failed'
            ? 'red'
            : inquiry.sla_status == 'Fulfilled'
              ? 'green'
              : 'orange'
        if (value == 'First Response Due' || value == 'Rolling Response Due') {
          value = __(timeAgo(inquiry.response_by))
          tooltipText = formatDate(inquiry.response_by)
          if (new Date(inquiry.response_by) < new Date()) {
            color = 'red'
          }
        }
        _rows[row] = {
        ...
        _rows[row] = {
          label: formatDate(inquiry[row]),
          timeAgo: __(timeAgo(inquiry[row])),
        }
      } else if (
        ...
        _rows[row] = {
          label: inquiry[field] ? formatDate(inquiry[field]) : '',
          timeAgo: inquiry[row]
            ? row == 'first_response_time'
              ? formatTime(inquiry[row])
              : __(timeAgo(inquiry[row]))
            : '',
        }
      }
    })
    ...
    _rows['_comment_count'] = inquiry._comment_count
    return _rows
  })
}

function onNewClick(column) {
  let column_field = inquiries.value.params.column_field
  ...
  router.push({ name: 'NewInquiry', query })
}
