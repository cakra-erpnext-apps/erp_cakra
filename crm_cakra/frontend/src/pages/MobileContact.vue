  ...
  <div v-if="contact.doc" class="flex flex-col h-full overflow-hidden">
    ...
    >
      ...
      <template #tab-panel="{ tab }">
        ...
        <InquiriesListView
          v-else-if="tab.label === 'Inquiries' && rows.length"
          class="mt-4"
          :rows="rows"
          :columns="columns"
...
const tabs = [
  ...
  },
  {
    name: 'Inquiries',
    label: __('Inquiries'),
    icon: h(InquiriesIcon, { class: 'h-4 w-4' }),
...
async function setAsPrimary(field, value) {
  let d = await call('crm_cakra.api.contact.set_as_primary', {
...
async function createNew(field, value) {
  if (!value) return
  let d = await call('crm_cakra.api.contact.create_new', {
    contact: contact.doc.name,
    field,
    value,
  })
