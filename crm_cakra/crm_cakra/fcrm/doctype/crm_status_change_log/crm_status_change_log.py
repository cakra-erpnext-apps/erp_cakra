...
def add_status_change_log(doc):
	to_status_type = frappe.db.get_value("CRM Inquiry Status", doc.status, "type") if doc.status else None

	if not doc.is_new():
		previous_status = doc.get_doc_before_save().status if doc.get_doc_before_save() else None
		previous_status_type = (
			frappe.db.get_value("CRM Inquiry Status", previous_status, "type") if previous_status else None
		)
